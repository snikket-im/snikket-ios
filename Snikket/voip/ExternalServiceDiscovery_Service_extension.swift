...
extension ExternalServiceDiscoveryModule.Service {
    ...
    private func escapedHost() -> String {
        if #available(iOS 12.0, *) {
            return IPv6Address(self.host) != nil ? "[\(self.host)]" : self.host;
        } else {
            return self.host.contains(":") && !(self.host.hasPrefix("[") || self.host.hasSuffix("]")) ? "[\(self.host)]" : self.host;
        }
    }
}
#endif
